...
function Weather({ weather, isLoading, isError }) {
  ...
  return (
    <div className='flex flex-col justify-between w-full h-[90vh] m-auto p-4 text-gray-300 z-10 border'>
        { weather?.main && !isLoading && !isError ?
            <div className='relative flex flex-col gap-12 max-w-[700px] w-full mx-auto text-gray-300 z-10 mt-8'>
                {/* Top */}
                <div className='relatie flex justify-between'>
                    ...
                    <p className='text-6xl sm:text-7xl md:text-7xl'><span className='text-3xl'>Temp</span> {weather?.main?.temp?.toFixed(0)}&#176;</p>
                </div>
                {/* Bottom */}
        ...
        <div className='flex gap-4 justify-between flex-wrap'>
            {constantCitiesWeather.map((city, index) => (           
                <div className='flex flex-col gap-4 items-center justify-center text-white relative rounded-md min-w-[170px]' key={index}>
                    <p className='text-3xl text-cente font-semibold'>{city?.name}</p>
                    <div className='flex gap-6 sm:gap-4 justify-center sm:justify-between items-center flex-wrap'>
                        <div className='text-center'>
                            <p className='text-lg sm:text-2xl font-bold'>{city?.main?.temp?.toFixed(0)}&#176;</p>
                            <p className='sm:text-lg'>Temp</p>
                        </div>
                    </div>
                </div>
            ))}
